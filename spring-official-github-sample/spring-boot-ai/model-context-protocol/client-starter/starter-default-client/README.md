...
Spring Boot 4 / Spring AI 2.0 command-line app showing the simplest possible MCP client setup using
the [MCP Client Boot Starter](https://docs.spring.io/spring-ai/reference/api/mcp/mcp-client-boot-starter-docs.html).

The application connects to a Brave Search MCP server over STDIO, injects all discovered tools into a `ChatClient`,
sends one question, prints the answer, and exits.
...
`ToolCallbackProvider` is auto-configured by the starter and exposes all MCP tools as Spring AI tool callbacks. The
application injects it directly into a `ChatClient`:
