...
This example demonstrates how a Model Context Protocol (MCP) server can dynamically update its available tools at
runtime, and how a client can detect and use these updated tools.

## Overview

The MCP protocol allows AI models to access external tools and resources through a standardized interface. This example
showcases a key feature of MCP: the ability to dynamically update the available tools on the server side, with the
client detecting and utilizing these changes.
...
1. **WeatherService**: Initially provides a weather forecast tool that retrieves temperature data for a specific
   location.
...
3. **Tool Discovery**: The client can query the available tools at any time, demonstrating that the tool list has been
   updated.
...
> The client implementation relies on the fact that the `ToolCallbackProvider#getToolCallbacks` implementation for MCP
> will always get the current list of MCP tools from the server.

This means that whenever a client requests the available tools, it will always get the most up-to-date list from the
server, without needing to restart or reinitialize the client.
