...
A cross-platform demo application showcasing Spring AI integration with the Model Context Protocol (MCP) Filesystem
server. This application enables natural language interactions with your local filesystem.

Connects to the [Filesystem MCP Server](https://github.com/modelcontextprotocol/servers/tree/main/src/filesystem) with
access to the `target` directory.

## Platform Support

✅ **Windows** - Automatic `cmd.exe` wrapper for npx ✅ **Linux** - Direct npx execution ✅ **macOS** - Direct npx
execution
...
**⚠️ Important:** When JSON configuration is enabled, the programmatic `@Bean` is automatically skipped via
`@ConditionalOnMissingBean` to avoid conflicts.

## Why Windows Requires Special Handling

On Windows, `npx` is implemented as a **batch file** (.cmd), not a native executable. Java's `ProcessBuilder` (used
internally by `StdioClientTransport`) cannot execute batch files directly.
...
**Solution:** Ensure the application is using the `cmd.exe` wrapper (should be automatic with the default programmatic
approach).
