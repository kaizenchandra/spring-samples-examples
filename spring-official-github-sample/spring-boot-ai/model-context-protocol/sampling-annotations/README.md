...
Two-module example showing MCP Sampling with Spring AI annotations. The server exposes a weather tool that uses MCP
Sampling to request poems from the client's LLMs; the client routes those sampling requests to OpenAI or Anthropic based
on model hints.
...
2. The server's `@McpTool` fetches live weather from Open-Meteo, then calls `exchange.createMessage()` twice — once
   hinting `openai`, once hinting `anthropic`.
