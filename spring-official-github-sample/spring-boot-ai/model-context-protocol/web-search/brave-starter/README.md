...
Demonstrates Spring AI's MCP client with
the [Brave Search MCP Server](https://github.com/modelcontextprotocol/servers/tree/main/src/brave-search) using Spring
Boot auto-configuration. On startup it asks one predefined question, prints the answer, and exits.

Unlike the manual approach, the MCP client is fully auto-configured via `application.properties` and
`mcp-servers-config.json` — no explicit bean definitions required.
